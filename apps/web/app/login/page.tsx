...
/**
 * Main entry point for the login experience.
 * Supports traditional email/password login, Google OAuth, and handles transition to MFA if required.
 */
export default function LoginPage() {
  ...
  const handleGoogleLogin = async () => {
    ...
    const { error: authError } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${globalThis.window?.location.origin}/auth/callback`,
      },
    });
    if (authError) {
      setError(authError.message);
        ...
        {!mfaRequired ? (
          <>
            ...
            <form onSubmit={handleLogin} className="space-y-4" noValidate>
              <div>
                <label htmlFor="email" className="block text-sm font-medium mb-1.5 text-gray-700">Email</label>
                <input
                  id="email"
                  type="email"
...
/**
 * Sub-component for handling the Multi-Factor Authentication (MFA) verification stage.
 * Separated to reduce cognitive complexity of the main LoginPage.
 * 
 * @param props.factorId - The ID of the enrolled MFA factor to challenge.
 * @param props.onSuccess - Callback triggered after successful verification.
 * @param props.onCancel - Callback to return to the email/password stage.
 */
function MfaChallengeForm({ factorId, onSuccess, onCancel }: { factorId: string, onSuccess: () => void, onCancel: () => void }) {
  const [code, setCode] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  /**
   * Handles the MFA verification process.
   * 
   * PSEUDOCODE: MFA Challenge & Verify
   * 1. Request an MFA challenge from Supabase for the specified factor.
   * 2. Submit the user-provided 6-digit code to verify the challenge.
   * 3. On success, trigger completion; otherwise, display error.
   */
    ...
    <>
      ...
      <form onSubmit={handleVerify} className="space-y-6">
        ...
        <button
          type="button"
          onClick={onCancel}
          className="on-full text-xs font-bold text-gray-400 hover:text-gray-600 transition-colors uppercase tracking-widest"
        >
          ← Back to Login
        </button>
      </form>
