...
describe('GoalQuery', () => {
  ...
  it('should get all goals', async () => {
    ...
    (from as any)._data = [{ id: '1', name: 'Car' }];
  ...
  it('should create a goal', async () => {
    const { from } = makeMockSupabase();
    (from as any)._data = { id: '2', name: 'House' };
    const query = new GoalQuery({ from } as any);
    const result = await query.create('user-1', { name: 'House', target_amount: 50000, currency: 'USD', deadline: '2030-01-01' });
    expect(result.name).toBe('House');
  });

  it('should update a goal', async () => {
    const { from } = makeMockSupabase();
    (from as any)._data = { id: '1', current_amount: 1000 };
    const query = new GoalQuery({ from } as any);
